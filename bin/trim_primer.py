...
def trim(reads, blast_df, margin_pc, min_read_length):
    ...
    primer_read_ids = set(blast_df['sseqid'].drop_duplicates())
    trimmed_seqs = []
    removed_seqs = []
    read_stats = []
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', "--margin_pc", help="primer must be at the start or end with this margin", default=20)
    parser.add_argument('-x', "--min_read_length", help="resulting read must be longer than this", default=50)
    args = parser.parse_args()
    margin = int(args.margin_pc)
