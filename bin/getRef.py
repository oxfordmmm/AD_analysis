...
def run_get_ref(taxid: str, bacteria_file: str, virus_file: str):
	...
	df=pd.read_csv(bacteria_file, sep='\t', skiprows=1, dtype=str)
	...
	df=pd.read_csv(virus_file, sep='\t', skiprows=1, dtype=str)
	df['species_taxid']=df['species_taxid'].map(str)
	df.dropna(subset=['ftp_path'],inplace=True)
	df=df.query('taxid==@taxid')
	if len(df)>=1:
		...
		with open('ref_path.txt', 'wt') as outfile:
			outfile.write(url)
		org_name=df['organism_name'].unique()[0]
		print(org_name)
		print(url)
		return org_name, url
