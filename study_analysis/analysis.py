...
def getDataFrame(input):
    ...
    for folder in input:
        ...
        dfs.append(df)
    df=pd.concat(dfs)
    # remove barcode from Sample name
...
def getMeta(meta, pathogens, pathogens_reduced,biofire,keep_runs):
    ...
    df['pathogen_1'] = df['pathogen_1'].str.strip()
    ...
    metaDict={}
    df['Run']=df['run_name']
    df['seq_name']=df['sample_name']
    df['Negative control']=np.where(df['pathogen_1']=='NEGATIVE',1,0)
    df['spiked']=np.where((df['SISPA']=='Y') & (df['cDNA'].isna()),0,1)
    ...
    metaDF.to_csv('metaDF_all.csv', index=False)
    ...
    for run in runs:
        ...
        for barcode in barcodes:
            ...
            Batch=metaDF[(metaDF['Run']==run) & (metaDF['barcode']==barcode)]['Batch'].unique()[0]
            NC=metaDF[(metaDF['Run']==run) & (metaDF['barcode']==barcode)]['Negative control'].unique()[0]
            test=metaDF[(metaDF['Run']==run) & (metaDF['barcode']==barcode)]['test'].unique()[0]
            spiked=metaDF[(metaDF['Run']==run) & (metaDF['barcode']==barcode)]['spiked'].unique()[0]
...
def main(args):
    ...
    metaDFbiofire_only['barcode']=metaDFbiofire_only['barcode'].map(int)
